...
import { CheckCircle, XCircle, Clock, MessageSquare, User, Phone, DollarSign } from 'lucide-react';
...
export function SolicitacoesPanel({ user }: SolicitacoesPanelProps) {
  ...
  const handleApproval = async (id: string, status: 'aprovado' | 'rejeitado') => {
    ...
    // Validar se é categoria Link e se o link foi preenchido
    ...
    try {
      await updateSolicitacao(id, status, observacoes[id] || '', links[id] || '');
      
      // Se aprovado, enviar para webhook
      if (status === 'aprovado' && solicitacao?.ticket) {
        const ticket = solicitacao.ticket;
        const atendente = atendentes.find(a => a.id === ticket.atendente_id);
        const atendenteNome = atendente?.nome || '';
        
        let webhookUrl = `http://fuzymc.com.br:5678/webhook-test/trello-ctn?atendente=${encodeURIComponent(atendenteNome)}&matricula=${encodeURIComponent(ticket.matricula)}&nome=${encodeURIComponent(ticket.nome)}&valor=${ticket.valor}&qtd=${ticket.qtd_mensalidades}&telefone=${ticket.telefone}&categoria=${ticket.categoria}`;
        
        // Adicionar link se for categoria Link
        if (ticket.categoria === 'Link' && links[id]) {
          webhookUrl += `&link=${encodeURIComponent(links[id])}`;
        }
      ...
      setSolicitacoes(prev => 
        prev.map(s => s.id === id ? { ...s, status, aprovado_por: user.id, link: links[id] } : s)
      );
  ...
  const filteredSolicitacoes = user.equipe === 'whatsapp' 
    ? solicitacoes
    : solicitacoes;
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        <div className="bg-gradient-to-r from-orange-600 to-orange-700 p-6">
          <h2 className="text-2xl font-bold text-white">
            {user.equipe === 'whatsapp' ? 'Aprovar Solicitações' : 'Minhas Solicitações'}
          </h2>
          <p className="text-orange-100 mt-1">
            {user.equipe === 'whatsapp' 
              ...
              <div className="grid gap-4">
                {filteredSolicitacoes.map((solicitacao) => (
                  ...
                  >
                    ...
                    {user.equipe === 'whatsapp' && solicitacao.status === 'pendente' && (
